...
def run_genetic_alg(
    ...
    Args:
        ...
        - eps: tolerance for unequal sized treatment arms

    Returns:
        optimized pool of treatment allocations and their fitness scores
    """
    init_pool_size = z_pool.shape[0]
    # n_arms = np.max(z_pool).astype(int) + 1
    scores = fitness_fn(z_pool)
    z_mean_bincounts = np.mean(
        np.vstack([np.bincount(z.astype(int)) for z in z_pool]), axis=0
    )
    ...
    for _ in tqdm(range(genetic_iters)):
        ...
        z_pool_chil_cross = np.vstack(
            [
                crossover(z_par1, z_par2, cross_k, rng)
                for (z_par1, z_par2) in zip(z_pool_par1_cross, z_pool_par2_cross)
            ]
            )
        z_pool_chil_copy = np.vstack(
            (z_pool_par1[which_cross == 0, :], z_pool_par2[which_cross == 0, :])
        )
