...
from src.sims.trial_loader import *
...
class SimulatedTrialConfig(ArgumentParser):
    ...
    def __init__(self):
        ...
        self.add_argument('--make-data', action='store_true', help='Flag for whether to generate data')
        self.add_argument('--run-trial', action='store_true', help='Flag for whether to run trial')
        self.add_argument('--analyze-trial', action='store_true', help='Flag for whether to analyze trial')
        self.add_argument('--n-arms', type=int, default=2, help='Number of arms in the trial')
        self.add_argument('--n-data-reps', type=int, default=4, help='Number of repeated data samples to generate')
        self.add_argument('--rep-to-run', type=int, default=0, help='Index of data sample to run trial on')

        self.add_argument('--data-dir', type=str, default='data', help='Directory to save/load data')
        self.add_argument('--out-dir', type=str, default='output', help='Directory to save output')
...
class SimulatedTrial(ABC):
    ...
    def __init__(self, config: Namespace):
        ...
        # Set configuration
        self.config = config
        self.loader = SimulatedTrialLoader(config)
    ...
    def set_data_from_config(self):
        ...
        if self.config.make_data:
            ...
            self.potential_outcome_mdl = self.loader.get_potential_outcome_mdl()
    ...
    def set_design_from_config(self):
        ...
        """
        self.rand_mdl = self.loader.get_design(self)
    ...
    def run_trial(self):
        ...
        self.observed_outcome_mdl = self.loader.get_observed_outcome_mdl(self)
    ...
    def analyze_trial(self):
        ...
        """
        estimator = self.loader.get_estimator(self)

        print('Getting treatment effect estimates')
        tau_hat_pool = [estimator.estimate(z, y_obs) for z, y_obs in tqdm(zip(self.z_pool, self.y_obs_pool), total=self.z_pool.shape[0])]

        print('Getting p-values')
        pvals = [estimator.get_pval(self.z_pool, self.y_obs_pool, idx) for idx in tqdm(range(self.z_pool.shape[0]))]
        ...
        if np.isscalar(self.tau_hat):
            ...
            self.rmse = np.sqrt(np.mean((self.tau_hat_pool - self.tau_true) ** 2, axis=0))
    ...
    def pickle(self):
        ...
        """
        attr_to_save = ['config', 'rr', 'bias', 'rmse', 'z_pool', 'tau_hat_pool', 'y_obs_pool', 'tau_true']
