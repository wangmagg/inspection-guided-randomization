#!/bin/bash
declare -a enum_arr=(10000 30000 50000 750000 100000 150000) 
declare -a accept_arr=(100 500 1000)  

for enum in "${enum_arr[@]}"
do
    for accept in "${accept_arr[@]}"
    do
        python3 -m vignettes.multarm --n-stu 80 --n-arms 4 --n-enum $enum --n-accept $accept --n-data-samples 5
    done
done

# 6 arms
python3 -m vignettes.multarm --n-stu 120 --n-arms 6 --tau-sizes 0 0.15 0.3 0.45 0.6 --n-enum 100000 --n-accept 500 --n-data-samples 5

# Make figures for n-arms:4
python3 -m vignettes.multarm_figs --n-stu 80 --n-arms 4 --n-enum 100000 --n-accept 500 --data-iter 0